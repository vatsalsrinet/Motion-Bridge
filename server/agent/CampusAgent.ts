...
import { AgentResponse, CampusLocation, SearchConstraints } from "../types/campus";
...
export class CampusAgent {
  ...
  async extractConstraints(query: string): Promise<SearchConstraints> {
    if (this.llmClient) {
      try {
        const raw = await this.llmClient.complete(
          ...
          'Allowed keys: category, openAfter (HH:MM), needsAccessibleEntrance, needsElevator, avoidActiveImpacts, maxDistanceMeters.',
        );
    ...
    return {
      category: q.includes("study") || q.includes("library") ? "study" : undefined,
  ...
  async generateExplanation(results: CampusLocation[]): Promise<string> {
    if (results.length === 0) return "I couldn't find a campus location that matches those requirements.";
  ...
  private validateConstraints(value: unknown): SearchConstraints {
    ...
    return {
      ...
      needsAccessibleEntrance: bool("needsAccessibleEntrance"), needsElevator: bool("needsElevator"), avoidActiveImpacts: bool("avoidActiveImpacts"),
